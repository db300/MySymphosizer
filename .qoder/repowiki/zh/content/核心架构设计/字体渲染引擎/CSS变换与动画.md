...
- 关键帧动画
  ...
  - **更新** 所有动画序列现在使用'YTUC'轴值替代旧的'vrsb'、'hght'、'ital'参数
  - 参考路径：[style.css:241-275](file://styles/style.css#L241-L275), [style.css:375-421](file://styles/style.css#L375-L421)
- 过渡动画
...
### 组件F：字体变形轴参数更新
- **更新** 字体变形轴参数从'vrsb'、'hght'、'ital'全面迁移至'YTUC'
- 实现细节
  - JavaScript端：通过splitChars[wornum].chars[i].style.fontVariationSettings = "'YTUC'" + ytuValue + ''设置轴值
  - CSS端：所有关键帧动画使用"YTUC"轴参数定义字符变形效果
  - 参数范围：ytuValue通过constrain(smoothH[i], 10, 100)限制在10-100范围内
- 影响范围
  - load-bounce-char、bounce-char、bounce-end等关键帧动画
  - 容器级字体变形设置：.container使用YTUC轴值初始化
  - 动态类型显示：.displaytop等元素的字体变形控制

**章节来源**
- [script.js:411-412](file://js/script.js#L411-L412)
- [style.css:241-275](file://styles/style.css#L241-L275)
- [style.css:375-421](file://styles/style.css#L375-L421)
- [style.css:860](file://styles/style.css#L860)
...
- **新增** 字体变形问题
  - 确认字体文件支持YTUC轴参数，检查fontVariationSettings格式是否正确
  - 验证ytuValue参数范围在10-100之间，避免超出字体支持范围
...
- **更新** 全面采用'YTUC'轴参数优化字体变形性能，提升GPU渲染效率
...
- **新增** 字体变形参数说明
  - YTUC轴参数范围：10-100（对应字体变形的细度变化）
  - 推荐值：10（纤细）到100（粗壮）
  - 性能优化：合理限制参数范围，避免过度变形影响渲染性能