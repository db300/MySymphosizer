...
|--------|----------|----------|----------|----------|
| `hght` | Height | -100 ~ +100 | 字形高度变化，控制垂直拉伸/压缩 | 音频强度、鼠标位置 |
| `ital` | Italic | 0 ~ 40 | 字体倾斜角度，模拟手写效果 | 音频强度阈值 |
| `vrsb` | Reverse | 0 ~ 1 | 文字方向翻转，创造镜像效果 | 切换按钮 |
...
PreProcess --> SmoothI[smoothI平滑]
PreProcess --> SmoothVol[音量平滑]
SmoothH --> MapH[数值映射]
SmoothI --> MapI[数值映射]
SmoothVol --> MapVol[音量映射]
MapH --> ApplyH[应用到hght轴]
MapI --> ApplyI[应用到ital轴]
MapVol --> ApplyVol[应用到vrsb轴]
ApplyH --> Output[最终字体变形]
ApplyI --> Output
ApplyVol --> Output
```
